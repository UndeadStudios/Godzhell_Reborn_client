public final class BZip2Decompressor {
    ...
    private static void method226(BZip2FileEntry BZip2FileEntry) {
        ...
        int save_nblock_pp = BZip2FileEntry.save_nblock + 1;
    ...
    private static void method227(BZip2FileEntry entry) {
        ...
        if (entry.tt == null) {
            entry.tt = new int[entry.blockSize100k * 0x186a0];
        }
        boolean flag19 = true;
        while (flag19) {
            ...
            for (int l3 = 0; l3 < j4; l3++) {
                ...
                method232(entry.limitt[l3], entry.base[l3], entry.perm[l3], entry.len[l3], byte8, i, i4);
            ...
            for (int k5 = ai2[l7 - ai1[i7]]; k5 != l4; )
                if (k5 == 0 || k5 == 1) {
                    ...
                    entry.tt[i6] = entry.seqToUnseq[byte6 & 0xff] & 0xff;
                    i6++;
                    if (j5 == 0) {
                        i5++;
                        j5 = 50;
            ...
            for (int l2 = 0; l2 < i6; l2++) {
                ...
                entry.tt[entry.cftab[byte7 & 0xff]] |= l2 << 8;
                entry.cftab[byte7 & 0xff]++;
            }

            entry.tPos = entry.tt[entry.origPtr] >> 8;
            entry.nblock_used = 0;
    ...
    private static byte readByte(BZip2FileEntry BZip2FileEntry) {
        return (byte) readBits(8, BZip2FileEntry);
    }

    private static byte readBit(BZip2FileEntry BZip2FileEntry) {
        return (byte) readBits(1, BZip2FileEntry);
    }

    private static int readBits(int i, BZip2FileEntry BZip2FileEntry) {
        int j;
        do {
    ...
    private static void method231(BZip2FileEntry BZip2FileEntry) {
        BZip2FileEntry.nInUse = 0;
        for (int i = 0; i < 256; i++)
            if (BZip2FileEntry.inUse[i]) {
                BZip2FileEntry.seqToUnseq[BZip2FileEntry.nInUse] = (byte) i;
                BZip2FileEntry.nInUse++;
            }
